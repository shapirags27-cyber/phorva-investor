...
app.post("/proof-statement", (req, res) => {
  ...
  if (Array.isArray(executions)) {
    try {
      ...
      /*
       * Build the verification trace directly from
       * Phorva's independently verified execution nodes.
       *
       * The trace is generated by Phorva and is never
       * accepted from the caller as authoritative input.
       */
          ...
          buildVerificationTrace({
            verification: {
              ...
              actualTransactionAmount:
                Number.isFinite(
                  Number(
                    node.analysis?.decodedParameters?.actualAmount ??
                    node.analysis?.decodedParameters?.amount
                  )
                )
                  ? Number(
                      node.analysis.decodedParameters.actualAmount ??
                  ...
                  : Number(node.transaction?.amount),
      ...
      let verifiedFinalState =
        null;

      let verifiedFinalStateCommitment =
        null;

      const finalStateRequested =
        finalStateVerification !== undefined ||
        finalStateCommitment !== undefined ||
        req.body?.finalState !== undefined;
      ...
      if (finalStateRequested) {
        ...
        verifiedAuthorization = {
          ...
          decision:
            verifiedAuthorization.decision === "AUTHORIZED" &&
            verifiedFinalState.satisfied === true
              ? "AUTHORIZED"
              : "BLOCKED"
        };
      ...
      return res.json({
        ...
        verification: {
          ...verifiedAuthorization,
          violations:
            verified.authorization.violations
        }
      });
    } catch (error) {
      return res.status(400).json({
...
app.post("/prove-execution", (req, res) => {
  ...
  try {
    /*
     * 1. Independently verify the raw execution evidence.
     */
    const verified =
      verifyRawExecutions({
        executions
    ...
    /*
     * 2. Build verification traces directly from
     * Phorva's verified execution nodes.
     */
    const verificationTraces =
      verified.nodes.map(node =>
        buildVerificationTrace({
    ...
    /*
     * 3. Optionally verify the requested final state.
     */
    ...
    /*
     * 4. Build the canonical Phorva proof statement.
     *
     * This is derived from independently verified
     * execution evidence, not caller assertions.
     */
    const proofStatement =
      buildPhorvaProofStatement({
        intent:
    ...
    /*
     * 5. Create the canonical statement commitment.
     */
    ...
    /*
     * 6. Send the canonical statement into the
     * provider-neutral prover adapter.
     */
    const proving =
      proveWithAdapter({
    ...
    /*
     * 7. Return the complete proof-carrying execution.
     */
    return res.json({
      version:
...
/*
 * Phorva Prover Adapter v1
 *
 * Provider-neutral interface for turning a canonical
 * Phorva proof statement into a proof request.
 *
 * Phorva defines the statement.
 * External proving infrastructure performs the proof.
 */
